...
import com.vikashparajuli.mycalendar.caldroid.infiniteviewpager.InfinitePagerAdapter;
import com.vikashparajuli.mycalendar.caldroid.infiniteviewpager.InfiniteViewPager;
...
public class CaldroidFragment extends DialogFragment {
     ...
     */
    public int prev;
    private static final int MONTH_YEAR_FLAG = DateUtils.FORMAT_ABBREV_MONTH
    ...
    private TextView monthPreviousTextView;
    private TextView monthNextTitleTextView;
    ...
    DATE_SELECTED="dateSelected",
            ...
            SELECTED_HALF_MOON_DATES = "selectedHalfMoonDates",
            SELECTED_FULL_MOON_DATES = "selectedFoolMoonDates",
            SELECTED_EVENT_DATES = "selectedEventDates",
            MIN_DATE = "minDate",
            MAX_DATE = "maxDate",
    ...
    protected ArrayList<DateTime> DateSelected = new ArrayList<>();

    protected ArrayList<DateTime> selectedFullMoons = new ArrayList<>();
    protected ArrayList<DateTime> selectedHalfMoons = new ArrayList<>();
    protected ArrayList<DateTime> selectedEvents = new ArrayList<>();
    protected DateTime minDateTime;
    protected DateTime maxDateTime;
    protected ArrayList<DateTime> dateInMonthsList;
    private ArrayList<String> fullmoon;
    ...
    public Map<String, Object> getCaldroidData() {
        ...
        caldroidData.put(DATE_SELECTED,DateSelected);
        caldroidData.put(SELECTED_HALF_MOON_DATES, selectedHalfMoons);
        caldroidData.put(SELECTED_FULL_MOON_DATES, selectedFullMoons);
        caldroidData.put(SELECTED_EVENT_DATES, selectedEvents);
    ...
    public void setBackgroundDrawableForDate(Drawable drawable, Date date) {
        DateTime dateTime = CalendarHelper.convertDateToDateTime(date);
        Log.e("date time...","date time.."+dateTime);
        backgroundForDateTimeMap.put(dateTime, drawable);
    }
    ...
    public Bundle getSavedStates() {
        ...
        if (selectedHalfMoons != null && selectedHalfMoons.size() > 0) {
            bundle.putStringArrayList(SELECTED_HALF_MOON_DATES,
                    CalendarHelper.convertToStringList(selectedHalfMoons));
        }

        if (selectedFullMoons != null && selectedFullMoons.size() > 0) {
            bundle.putStringArrayList(SELECTED_FULL_MOON_DATES,
                    CalendarHelper.convertToStringList(selectedFullMoons));
        }

        if (selectedEvents != null && selectedEvents.size() > 0) {
            bundle.putStringArrayList(SELECTED_EVENT_DATES,
                    CalendarHelper.convertToStringList(selectedEvents));
        }
    ...
    public void clearFullMoonDates() {
        selectedFullMoons.clear();
    }

    public void clearHalfMoonDates() {
        selectedHalfMoons.clear();
    }

    public void clearEventDates() {
    ...
    public void setSelectedHalfMoonDates(Date date) {
        if (date == null) {
            return;
        }
        DateTime dateTime = CalendarHelper.convertDateToDateTime(date);
        selectedHalfMoons.add(dateTime);
    }

    public void setSelectedFullMoonDates(Date date) {
        if (date == null) {
            return;
        }
        DateTime dateTime = CalendarHelper.convertDateToDateTime(date);
        selectedFullMoons.add(dateTime);
    }



    public void setFullmoon(ArrayList<String> fullmoon) {
        for (int i = 0; i != fullmoon.size(); i++) {
            if (fullmoon.get(i).length() == 1) {
                fullmoon.set(i, "0" + fullmoon.get(i));
            }
        }
    ...
    public void setSelectedEvents(Date date) {
        if (date == null) {
            return;
        }
        DateTime dateTime = CalendarHelper.convertDateToDateTime(date);
        selectedEvents.add(dateTime);
    }
    ...
    public OnItemClickListener getDateItemClickListener() {
        ...
        if (dateItemClickListener == null) {
            dateItemClickListener = new OnItemClickListener() {
                ...
                //    setBackgroundDrawableForDate(getResources().getDrawable(R.drawable.mark_selected),dateTime);
                    setBackgroundDrawableForDateTime(getResources().getDrawable(R.drawable.mark_selected),dateTime);
    ...
    protected void refreshMonthTitleTextView() {
        ...
        String monthPrevTitle = DateUtils.formatDateRange(getActivity(),
                monthYearFormatter, millisPrev, millisPrev, MONTH_YEAR_FLAG).toString();

        monthPreviousTextView.setText(monthPrevTitle);
        ...
        // Refresh Next month title
        firstMonthTime.year = year;
        firstMonthTime.month = month;
        firstMonthTime.monthDay = 15;
        long millisNext = firstMonthTime.toMillis(true);

        // This is the method used by the platform Calendar app to get a
        // correctly localized month name for display on a wall calendar
        monthYearStringBuilder.setLength(0);
        String monthNextTitle = DateUtils.formatDateRange(getActivity(),
                monthYearFormatter, millisNext, millisNext, MONTH_YEAR_FLAG).toString();

        monthNextTitleTextView.setText(monthNextTitle);
    ...
    protected void retrieveInitialArgs() {
        ...
        if (args != null) {
            ...
            // Get Half moon dates
            ArrayList<String> selectedHalfMoonDateStrings = args.getStringArrayList(SELECTED_HALF_MOON_DATES);
            if (selectedHalfMoonDateStrings != null && selectedHalfMoonDateStrings.size() > 0) {
                selectedHalfMoons.clear();
                for (String dateString : selectedHalfMoonDateStrings) {
                    DateTime dt = CalendarHelper.getDateTimeFromString(
                            dateString, null);
                    selectedHalfMoons.add(dt);
                }
            }

            // Get Full moon dates
            ArrayList<String> selectedFullMoonDateStrings = args.getStringArrayList(SELECTED_FULL_MOON_DATES);
            if (selectedFullMoonDateStrings != null && selectedFullMoonDateStrings.size() > 0) {
                selectedFullMoons.clear();
                for (String dateString : selectedFullMoonDateStrings) {
                    DateTime dt = CalendarHelper.getDateTimeFromString(dateString, null);
                    selectedFullMoons.add(dt);
                }
            }

            // Get Events dates
            ArrayList<String> selectedEventDateStrings = args.getStringArrayList(SELECTED_EVENT_DATES);
            if (selectedEventDateStrings != null && selectedEventDateStrings.size() > 0) {
                selectedEvents.clear();
                for (String dateString : selectedEventDateStrings) {
                    DateTime dt = CalendarHelper.getDateTimeFromString(dateString, null);
                    selectedEvents.add(dt);
                }
            }
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        monthPreviousTextView = (TextView) view
                .findViewById(R.id.calendar_previous_month_year_textview);
        monthNextTitleTextView = (TextView) view
                .findViewById(R.id.calendar_next_month_year_textview);
